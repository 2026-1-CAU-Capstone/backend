...
코드를 생성하고 난 후에는 반드시 document를 생성해서 사람이 읽고 이해할 수 있도록 설명하고 docs/agent 경로에 저장한다. 이때 제목에는 타임스탬프가 포함되어야 한다.
내용에는 작업한 내용, 설계의도, 제대로 지시되지 않아 임의로 결정하고 행동한 부분, 개발자가 알아둬야하는 내용이 포함되어야 한다.

클래스를 생성한경우 각 클래스가 무슨 역할을 하는지에 대한 표와 클래스간 논리흐름도를 mermaid를 이용하여 같이 정리해서 작성해야 한다.

API를 수정한 경우 swagger 명세도 같이 업데이트 한다
## 프로젝트 개요
