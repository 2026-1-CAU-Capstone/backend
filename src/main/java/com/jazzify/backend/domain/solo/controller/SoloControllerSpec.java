...
@NullMarked
@Tag(name = "Solo", description = "솔로(Solo) CRUD API — 재즈 솔로 저장·조회·수정·삭제")
public interface SoloControllerSpec {
		...
		requestBody = @RequestBody(
			content = @Content(
				...
				examples = @ExampleObject(
					name = "Charlie Parker — ii-V-I Bebop Solo (알토 색소폰, 비밥) — 특수 기능 전체 포함",
	...
	ApiResponse<SoloResponse> create(SoloCreateRequest request);

	@Operation(
		summary = "솔로 목록 조회 (페이징)",
		description = """
			솔로 목록을 페이징하여 반환합니다.
	...
	ApiResponse<SoloResponse> getByPublicId(UUID publicId);

	@Operation(
		summary = "솔로 수정",
		description = """
			솔로의 내용을 수정합니다. **(PUT — 전체 교체 방식)**
	...
	ApiResponse<SoloResponse> update(UUID publicId, SoloUpdateRequest request);

	@Operation(
		summary = "솔로 삭제",
		description = """
				`publicId`에 해당하는 솔로를 삭제합니다.
	...
	@Operation(
		summary = "솔로 영상 연결/갱신",
		description = """
			솔로에 영상을 연결하거나 기존 영상을 갱신합니다. **(PUT — upsert 방식)**
	...
	ApiResponse<SoloResponse> updateVideo(UUID publicId, SoloVideoRequest request);

	@Operation(
		summary = "솔로 영상 연결 해제",
		description = """
			솔로에 연결된 영상 정보를 삭제합니다.
	...
	@Operation(
		summary = "OMR로 솔로 생성 (악보 파일 업로드)",
			description = """
				악보 이미지 파일을 MusicVision OMR 서버에 비동기로 제출하고 `isOMR=true`인 Solo를 생성합니다.
