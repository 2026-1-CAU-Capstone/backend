...
/**
 * 릭(Lick)에 연결된 영상 정보 엔티티.
 * <p>
 * 하나의 릭에 여러 영상 구간(YouTube, Spotify, SoundCloud 등)을 연결할 수 있다.
 */
@Entity
@Table(name = "tb_lick_video")
@Getter
@NullMarked
@Builder
@NoArgsConstructor(access = AccessLevel.PROTECTED)
@AllArgsConstructor(access = AccessLevel.PRIVATE)
public class LickVideo extends BaseEntity {

	/** 부모 릭 */
	@OneToOne(fetch = FetchType.LAZY, optional = false)
	@JoinColumn(name = "lick_id", nullable = false, unique = true)
	private Lick lick;
