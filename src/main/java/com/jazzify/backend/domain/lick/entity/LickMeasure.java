 ...
 */
@Entity
@Table(name = "tb_lick_measure")
@Getter
@NullMarked
@Builder
@NoArgsConstructor(access = AccessLevel.PROTECTED)
@AllArgsConstructor(access = AccessLevel.PRIVATE)
public class LickMeasure extends BaseEntity {

	/** 부모 릭 */
	@ManyToOne(fetch = FetchType.LAZY, optional = false)
	@JoinColumn(name = "lick_id", nullable = false)
	private Lick lick;
	...
	private List<LickNote> notes = new ArrayList<>();

	// ─── Helpers ────────────────────────────────────────────────────────

	/** 음표를 이 마디에 추가한다. 양방향 관계를 동기화한다. */
	public void addNote(LickNote note) {
		notes.add(note);
	}
}
